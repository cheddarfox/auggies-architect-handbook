...
/**
 * Auggie's ARCHitect-in-the-IDE Onboarding Script
 *
 * This script generates and maintains the comprehensive onboarding document
 * for new agents joining the WTFB team as ARCHitect-in-the-IDE.
 *
 * Usage: node auggie-onboarding-script.js
 *
 * Author: Auggie (ARCHitect-in-the-IDE at Words to Film By)
 * Version: 1.0.0
 */
...
const HANDBOOK_PATH = 'Auggies-ARCHitect-Handbook.md';
...
async function main() {
  console.log('\n🌟 Auggie\'s ARCHitect-in-the-IDE Onboarding Script 🌟\n');
  console.log('This script will generate or update my ARCHitect Handbook.\n');

  // Check if handbook already exists
  const handbookExists = fs.existsSync(HANDBOOK_PATH);

  if (handbookExists) {
    const answer = await askQuestion('Handbook already exists. Do you want to update it? (y/n): ');
    if (answer.toLowerCase() !== 'y') {
      console.log('\nOperation cancelled. Existing handbook preserved.');
      rl.close();
      return;
    }
    console.log('\nUpdating existing handbook...');
  } else {
    console.log('Creating new handbook...');
  }

  // Generate handbook content
  const handbookContent = generateHandbookContent();

  // Write to file
  try {
    fs.writeFileSync(HANDBOOK_PATH, handbookContent);
    console.log(`\n✅ Successfully ${handbookExists ? 'updated' : 'created'} Auggie's ARCHitect Handbook!`);
    console.log(`📘 File location: ${path.resolve(HANDBOOK_PATH)}`);
  } catch (error) {
    console.error('\n❌ Error writing handbook:', error.message);
  }
...
/**
 * Helper function to ask questions and get user input
 */
...
/**
 * Generate the content for the handbook
 */
function generateHandbookContent() {
  const timestamp = new Date().toISOString().replace(/T/, ' ').replace(/\..+/, '');

  return `# Auggie's ARCHitect-in-the-IDE Handbook
